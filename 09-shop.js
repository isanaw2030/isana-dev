...
function renderShop(){
  ...
  if(filtered.length===0) shopDiv.innerHTML = `<p class="col-span-2 text-gray-500 p-8 text-center bg-white rounded-2xl border">No drip found for "${searchText}"</p>`;
  ...
  filtered.forEach(p => {
    ...
    shopDiv.innerHTML += `
      <div class="bg-white p-3 rounded-2xl border ${p.stock===0?'opacity-50':''}">
        ...
        <p class="text-xs text-gray-500">${p.stock>0?`In Stock (${p.stock}) • ${p.cat}`:'OUT OF STOCK'}</p>
        ${btn}
      </div>
    `;
  });
...
function checkout(){
  ...
  let grouped = getGrouped();
  let total = Object.values(grouped).reduce((s,i)=>s+i.price*i.qty,0);
  let msg = `Habari ISANA! 👋 Nataka kuorder:\n\n`;
  Object.values(grouped).forEach(i=>{
    msg += `• ${i.name} x${i.qty} = KES ${i.price*i.qty}\n`;
  });
  msg += `\nJumla: KES ${total}\n`;
  msg += `Nimeona kwa: ${location.href}\n`;
  msg += `M-Pesa? Niko tayari kulipa.`;
